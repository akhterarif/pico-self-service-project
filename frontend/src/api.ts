...
export type Vm = { id: number; name: string; package: Package; cloud_server_id: string; status: string; ip_address?: string; created_at: string; updated_at: string; latest_invoice_id?: number, next_billing_date: string, usage?: { storage: number; cpu: number; memory: number; network: number } };
